        ...
        if (this.enabled)
        {
            ...
            else
            {
                ...
                if (txt.GetComponent<TextMesh>().text == correct_code)
                {
                    ...
                    ind.renderer.material.SetColor("_Color", new Color(0.235f, 0.235f, 0.235f));
                    ind.renderer.material.SetColor("_SpecColor", new Color(0, 1, 0));
                ...
                else
                {
                    ...
                    ind.renderer.material.SetColor("_Color", new Color(0.235f, 0.235f, 0.235f));
                    ind.renderer.material.SetColor("_SpecColor", new Color(1, 0, 0));
                    //снова задержка, чтобы пользователь успел заметить
                    StartCoroutine(Wait(1.5f));
                    //стираем строку и возвращаем индикатору исходный цвет
                    txt.GetComponent<TextMesh>().text = "";
                    ind.renderer.material.SetColor("_Color", new Color(0, 0, 0));
                    ind.renderer.material.SetColor("_SpecColor", new Color(0.235f, 0.235f, 0.235f));
                }

...
public class AttackMoveController : MonoBehaviour
{
    ...
    public float targetDistanceFront = 2.5f, targetDistanceBack = 1;
    ...
    void Update()
    {
        ...
        var playerToCharacterDirection = (character.position - player.position);
        ...
        var degreesFromPlayerForward = Vector3.Angle(playerToCharacterDirection, player.forward);
        ...
        var behindFactor = Mathf.InverseLerp(0, 90, degreesFromPlayerForward);

        // Lerp the target distance between the front and back target distances based on the behindFactor
        var targetDistance = Mathf.Lerp(targetDistanceFront, targetDistanceBack, behindFactor);

        // Calculate the targetPosition
        var targetPosition = player.position + playerToCharacterDirection * targetDistance;
        ...
        var targetVector = targetPosition - character.position;
        targetVector.y = 0;

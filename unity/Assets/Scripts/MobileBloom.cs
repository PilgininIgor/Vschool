...
[ExecuteInEditMode, RequireComponent(typeof(Camera)), AddComponentMenu("Image Effects/Mobile Bloom")]
public class MobileBloom : MonoBehaviour
{

    public float intensity = 0.5f, colorMixBlend = 0.25f, agonyTint = 0.0f;
    ...
    void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        var tempRtLowA = RenderTexture.GetTemporary(source.width / 4, source.height / 4, (int)rtFormat);
        var tempRtLowB = RenderTexture.GetTemporary(source.width / 4, source.height / 4, (int)rtFormat);

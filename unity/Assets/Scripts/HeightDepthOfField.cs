...
[ExecuteInEditMode, RequireComponent(typeof(Camera)), AddComponentMenu("Image Effects/Height Depth of Field")]
public class HeightDepthOfField : MonoBehaviour
{
    ...
    public float maxBlurSpread = 1.55f, foregroundBlurExtrude = 1.055f, smoothness = 1.0f;
    ...
    private float widthOverHeight = 1.25f, oneOverBaseSize = 1.0f / 512.0f;

    private float cameraNear = 0.5f, cameraFar = 50.0f, cameraFov = 60.0f, cameraAspect = 1.333333f;
    ...
    void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        var mediumTexture2 = RenderTexture.GetTemporary(source.width / divider, source.height / divider, 0);
        var lowTexture = RenderTexture.GetTemporary(source.width / (divider * 2), source.height / (divider * 2), 0);
    ...
    {
        var tmp = RenderTexture.GetTemporary(to.width, to.height, 0);

        if (iterations < 2)
        {
            dofBlurMaterial.SetVector("offsets", new Vector4(0, spread * oneOverBaseSize, 0, 0));
            tmp.DiscardContents();
            Graphics.Blit(from, tmp, dofBlurMaterial, blurPass);

            dofBlurMaterial.SetVector("offsets", new Vector4(spread / widthOverHeight * oneOverBaseSize, 0, 0, 0));
            to.DiscardContents();
        ...
        else
        {
            ...
            dofBlurMaterial.SetVector("offsets", new Vector4(spread / widthOverHeight * oneOverBaseSize, spread * oneOverBaseSize, 0, 0));
            tmp.DiscardContents();

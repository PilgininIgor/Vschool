...
public class KamikazeMovementMotor : MovementMotor {
	...
	public float flyingSpeed = 5.0f, zigZagness = 3.0f, zigZagSpeed = 2.5f,
        oriantationMultiplier = 2.5f, backtrackIntensity = 0.5f;
	
	private Vector3 smoothedDirection = Vector3.zero;
			
	void FixedUpdate () {
		var dir = movementTarget - transform.position;
		var zigzag = transform.right * (Mathf.PingPong (Time.time * zigZagSpeed, 2.0f) - 1.0f) * zigZagness;
		...
		var deltaVelocity = (smoothedDirection * flyingSpeed + zigzag) - GetComponent<Rigidbody>().velocity;		

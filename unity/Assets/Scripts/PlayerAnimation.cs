...
Rigidbody rigid;
Transform rootBone, upperBodyBone;
float maxIdleSpeed = 0.5f, minWalkSpeed = 2.0f;
AnimationClip idle,turn, shootAdditive;
MoveAnimation[] moveAnimations;
SignalSender footstepSignals;

private Transform tr;
private Vector3 lastPosition = Vector3.zero, velocity = Vector3.zero, localVelocity = Vector3.zero;
private float speed = 0, angle = 0, lowerBodyDeltaAngle = 0, idleWeight = 0;
private Vector3 lowerBodyForwardTarget = Vector3.forward, lowerBodyForward = Vector3.forward;
...
void Awake () {
	...
	foreach (var moveAnimation in moveAnimations) {
		moveAnimation.Init ();
...
void Update () {
	...
	if (bestAnimation != null && idleWeight < 0.9) {
		var newAnimTime = Mathf.Repeat ((float) (animationComponent[bestAnimation.clip.name].normalizedTime * 2 + 0.1), 1);

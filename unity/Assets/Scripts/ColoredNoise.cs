...
[ExecuteInEditMode, RequireComponent(typeof(Camera)), AddComponentMenu("Image Effects/Noise")]
public class ColoredNoise : MonoBehaviour
{

    public float globalNoiseAmount = 0.075f, globalNoiseAmountOnDamage = -6.0f, localNoiseAmount = 0.0f;
    ...
    static void DrawNoiseQuadGrid(RenderTexture source, RenderTexture dest, Material fxMaterial, Texture2D noise, int passNr)
    {
        ...
        for (float x1 = 0; x1 < 1.0; x1 += stepSizeX)
        {
            for (float y1 = 0; y1 < 1.0; y1 += stepSizeY)
            {

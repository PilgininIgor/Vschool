...
namespace ILS.Domain.Migrations
{
    ...
    public sealed class Configuration : DbMigrationsConfiguration<ILS.Domain.ILSContext>
    {
        ...
        protected override void Seed(ILS.Domain.ILSContext context)
        {
            ...
            var adminRole = context.Role.Add(new Role() { Name = "Admin" });
            var teacherRole = context.Role.Add(new Role() { Name = "Teacher" });
            var studentRole = context.Role.Add(new Role() { Name = "Student" });
            context.User.Add(new User()
            {
                Name = "Supervisor",
                PasswordHash = CalculateSHA1("Supervisor"),
                Email = "supervisor@mail.ru",
                Roles = new List<Role> { adminRole, teacherRole, studentRole },
                IsApproved = true
            });
            context.User.Add(new User()
            {
            ...
            context.User.Add(new User()
            {
                ...
                LastName = "Александр",
                PasswordHash = CalculateSHA1("password"),
                Email = "grigorievalexandr@mail.ru",
                Roles = new List<Role> { adminRole, teacherRole, studentRole },
                IsApproved = true
            });
            context.User.Add(new User()
            {
                ...
                PasswordHash = CalculateSHA1("password"),
                Email = "konopelkindmitriy@mail.ru",
                Roles = new List<Role> { adminRole, teacherRole, studentRole },
                IsApproved = true
            });
            context.User.Add(new User()
